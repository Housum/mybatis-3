...
public class XMLConfigBuilder extends BaseBuilder {
    ...
    private Properties settingsAsProperties(XNode context) {
        ...
        for (Object key : props.keySet()) {
            //检查配置中心是否有该配置的set方法 如果木有的话 那么说明该配置存在问题
    ...
    /**
     * https://mybatis.org/mybatis-3/configuration.html#databaseIdProvider
     *
     * 作用：能够根据不同的数据库执行不同的语句，比如：
     * <databaseIdProvider type="DB_VENDOR">
     *   <property name="SQL Server" value="sqlserver"/>
     *   <property name="DB2" value="db2"/>
     *   <property name="Oracle" value="oracle" />
     * </databaseIdProvider>
     * 如果当前执行的环境是DB2，那么将会执行DB2的语言
     */
    ...
    private void mapperElement(XNode parent) throws Exception {
        if (parent != null) {
            for (XNode child : parent.getChildren()) {
                //如果是包名 那么扫描该包下面的类 这种方式下 是通过注解的方式实现的 不再扫描xml文件了
                //TODO 这部分在看完xml解析之后方式之后再看
                if ("package".equals(child.getName())) {
